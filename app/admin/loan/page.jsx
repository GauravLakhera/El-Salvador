...
// Modal for confirming deletion of a loan entry
    ...
    >
      ...
      >
        ...
        <p className="text-gray-700 mb-6">
          ¿Estás seguro de que quieres eliminar la solicitud de préstamo de "
          {item.name}"? Esta acción no se puede deshacer.
        </p>
...
const LoanAdminPage = () => {
  ...
  const fetchLoanEntries = async () => {
    ...
    try {
      ...
      if (result.data && Array.isArray(result.data)) {
        setLoanEntries(result.data);
      } else {
        console.warn(
          "API response for loan entries fetch was successful but 'data' array was missing or invalid:",
          result
        );
        setLoanEntries([]); // Ensure it's an empty array
        setError(
          result.message ||
            "Formato de respuesta de la API inesperado al cargar las solicitudes de préstamo."
        );
      }
    } catch (err) {
      console.error("Error al cargar las solicitudes de préstamo:", err);
      setError(
        "No se pudieron cargar las solicitudes de préstamo. Por favor, inténtalo de nuevo más tarde."
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleConfirmDelete = async (id) => {
    ...
    try {
      const response = await fetch(
        `https://el-salvador-backend.onrender.com/api/v1/loans/${id}`,
        {
          method: "DELETE",
        }
      );
      const result = await response.json();
      if (result.success) {
        setLoanEntries((prevEntries) =>
          prevEntries.filter((entry) => entry._id !== id)
        );
      } else {
        setError(
          result.message || "Error al eliminar la solicitud de préstamo."
        );
      }
    } catch (err) {
      console.error("Error al eliminar solicitud de préstamo:", err);
      setError("Error de red o del servidor al eliminar la solicitud.");
    } finally {
      setLoading(false);
          ...
          <div className="flex items-center justify-between py-6">
            <div>
              ...
              <p className="text-gray-600 mt-1 text-xs md:text-sm">
                Revisa y gestiona las solicitudes de préstamo de los usuarios.
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {loading && (
          <p className="text-center text-indigo-700 text-xl py-10">
            Cargando solicitudes de préstamo...
          </p>
        )}
        {error && (
        ...
        {!loading && !error && loanEntries.length === 0 && (
          <div className="text-center py-10">
            <p className="text-gray-600 text-lg">
              No hay solicitudes de préstamo para mostrar.
            </p>
          </div>
        )}
            ...
            {loanEntries.map((entry) => (
              ...
              >
                <h3 className="text-xl font-bold text-gray-900 mb-3">
                  <User size={20} className="inline mr-2 text-gray-500" />
                  {entry.name}
                </h3>
                ...
                <div className="flex justify-end mt-4 pt-4 border-t border-gray-100">
                  <motion.button
                    ...
                    aria-label={`Eliminar solicitud de ${entry.name}`}
